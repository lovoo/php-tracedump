...
class Html
{
    ...
    function dieobj_class($data, $class = null, $refl = null)
    {
        ...
        $end[] = array(
            "prop" => " ",
            "name" => "<b><span style='color:darkblue'>class</span> <big>".$this->dievalue_class(
                    $data,
                    $class
                )."</big></b>",
    ...
    function dieobj_methods($data, $class = null, $refl = null)
    {
        ...
        foreach ($data as $k => $v) {
            ...
            if ($doc = $v->getDocComment()) {
                $doc = " <span title=\"".htmlspecialchars(
                        $doc
                    )."\" style='cursor:help;color:rgb(128,128,64)'>/* doc */</span>";
            }
    ...
    function dietable($data)
    {
        ...
        foreach ($data as $fe) {
            ...
            if ($value) {
                if ($type == "array") {
                    $name = "<div style='float:left;'>".$name."</div>"."<div style='float:left;'><pre style='margin:0;padding:0'>".trim(
                            $value
                        )."</pre></div>";
                } else {
                    $name = "<div style='float:left;margin-right:5px'>".$name." =</div>"."<div style='float:left;'><pre style='margin:0;padding:0'>".trim(
                            $value
                        )."</pre></div>";
                }
    ...
    function dievalue_class($data, $class = null)
    {
        ...
        $link = "<a href='".$path."/dd.php?readfile=".urlencode(
                $classLink
            )."' target='href' style='color:black;text-decoration:none'><small>".$className."</small></a>";
